...
def getInfo(keyWord, pageNum):
    ...
    with codecs.open(fileName, 'w', 'utf-8') as fp:   ###用关键词做为保存的文件名
        ...
        for i in range(len(info['records'])):
            ...
            pbyear = int(info['records'][i]['publicationYear'])
            if((pbyear >= 2017 )or(pbyear < 1995)):      ##########更新内容，截取年份
                # print("0.lentitle = %d"%len(pagetitle))
                # print("0.lenyear = %d"%len(pageyear) )
                # print("0.lenkwd = %d" %len(pagekwd))
                continue
            pageyear.append(pbyear)  #####pageyear保存year
            f22.write("%d\n" %pageyear[totalnum])
            ...
            try:
                ...
                pageyear.pop()
                pagetitle.pop()
                # print("1.lentitle = %d" % len(pagetitle))
                # print("1.lenyear = %d" % len(pageyear))
                # print("1.lenkwd = %d" % len(pagekwd))
                continue
            if re.search('document', documentLink):
                ...
                pageyear.pop()
                pagetitle.pop()
                # print("2.lentitle = %d" % len(pagetitle))
                # print("2.lenyear = %d" % len(pageyear))
                # print("2.lenkwd = %d" % len(pagekwd))
                continue
            try:
                ...
                pageyear.pop()
                pagetitle.pop()
                # print("3.lentitle = %d" % len(pagetitle))
                # print("3.lenyear = %d" % len(pageyear))
                # print("3.lenkwd = %d" % len(pagekwd))
                continue
            #####不用摘要，被降维成关键字了#####
            # try:
            ...
            try:
                ...
                pagetitle.pop()
                pageyear.pop()
                # print("4.lentitle = %d" % len(pagetitle))
                # print("4.lenyear = %d" % len(pageyear))
                # print("4.lenkwd = %d" % len(pagekwd))
                continue
            else:
            ...
            time.sleep(1+random.random()) ###手动加入延迟，避免速度过快被封延迟时间在1s-3s之间
            # fp.write('\r\n'*6)
            totalnum += 1
            print('totalnum = %d\n' %totalnum)
...
if __name__ == '__main__':
    ...
    ##################1.读入数据1##################
    f11name = keyWord + '_title.txt'
    f22name = keyWord + '_year.txt'
    f33name = keyWord + '_keywords.txt'
    f11 = codecs.open(f11name, 'w', 'utf-8')
    f22 = codecs.open(f22name, 'w', 'utf-8')
    f33 = codecs.open(f33name, 'w', 'utf-8')
    ...
    # fff = pd.DataFrame(pp, index=[clusters], columns=['year'])  #####啥意思？？？？？？
    # print(fff)
